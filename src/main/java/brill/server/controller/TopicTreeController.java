...
public class TopicTreeController {
    ...
    *  {"event": "subscribe", "topic": "/myApp", "filter": {includeFileContent: true}}
    *   
    */
    ...
    public void subscribeToTopicTree(@Session WebSocketSession session, @Message JsonObject message) throws WebSocketException {
        ...
        try {
            ...
            if (filterObj != null && filterObj.containsKey("includeFileContent") &&
                filterObj.get("includeFileContent").getValueType().name().equals("TRUE")) {
                    includeFileContent = true;
            }

            String content = gitService.getFileTree(wsService.getWorkspace(session), topic, includeFileContent).toString();
